...
import { listBisnosPaginatedUseCase } from "@src/application/use-cases/composition";
...
export const listBisnosHandler = async (
  req: express.Request,
  ...
  try {
    ...
    const { data, ...paginated } =
      await listBisnosPaginatedUseCase.execute(filters);
    ...
    return res.status(500).json({
      ...
      meta: {
        errors: [
          {
            field: undefined,
            error: "An unexpected error occurred while processing bisno list",
          },
        ],
      },
    } satisfies IApiResponse);
